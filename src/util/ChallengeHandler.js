...
function Injector() {
	...
	this.answer = async (choice, empty) => {
		...
		const question =
			this.challengeData.kahoot.questions[this.data.questionIndex];
		let tick = Date.now() - this.receivedQuestionTime;
		if (
			this.defaults.options.ChallengeGetFullScore ||
			this.defaults.options.ChallengeWaitForInput ||
			!this.challengeData.challenge.game_options.question_timer
		) {
			tick = 1;
		}
		const pointsQuestion =
			this.challengeData.progress.questions.reverse()[0].pointsQuestion ||
			false;
		...
		const timeScore =
			+this.defaults.options.ChallengeScore ||
			Math.round((1 - tick / question.time / 2) * 1000) *
				question.pointsMultiplier *
				+pointsQuestion;
		if (this.data.streak === -1) {
			this.data.streak = 0;
			const ent =
				this.challengeData.progress.playerProgress
					.playerProgressEntries;
			let falseScore = 0;
			for (let q in ent) {
				...
				if (
					ent[q].questionMetrics[this.name] > falseScore ||
					!this.challengeData.kahoot.questions[q].points
				) {
					this.data.streak++;
				} else {
					this.data.streak = 0;
		...
		switch (question.type) {
			...
			case "jumble": {
				correct =
					JSON.stringify(choice) === JSON.stringify([0, 1, 2, 3]);
				if (
					typeof choice != "object" ||
					typeof choice.length === "undefined"
				) {
					choice = [];
				}
				for (let j in choice) {
			...
			case "open_ended": {
				...
				for (let choice of question.choices) {
					if (choice.answer.replace(emoji, "")) {
						correct =
							test.replace(emoji, "").toLowerCase() ==
							choice.answer
								.replace(emoji, "")
								.replace(invalid, "")
								.toLowerCase();
			...
			default: {
				...
				text =
					(question.choices[choiceIndex] &&
						question.choices[choiceIndex].answer) ||
					"";
				correct = true;
				if (this.defaults.options.ChallengeScore) {
					score += timeScore;
		...
		if (!correct && empty === null) {
			this.data.unansweredCount++;
		}
		if (!correct && empty !== null) {
		...
		const event = {
			...
			lastGameBlockIndex:
				this.data.questionIndex < 0 ? 0 : this.data.questionIndex - 1,
		};
		this.data.finalResult = {
			...
			isGhost: false,
			playerCount:
				this.challengeData.challenge.challengeUsersList.length + 1,
			startTime: this.challengeData.progress.timestamp,
			quizId: this.challengeData.kahoot.uuid,
			name: this.name,
			totalScore: this.data.totalScore,
		...
		return this._httpRequest(
			`https://kahoot.it/rest/challenges/${this.challengeData.challenge.challengeId}/answers`,
			{
				headers: {
	...
	this.next = () => {
		...
		switch (this.data.phase) {
			...
			case "ready": {
				this._getProgress(this.data.questionIndex)
					.then((inf) => {
						if (this.data.hitError === true) {
							this.data.hitError = false;
						}
						...
						let q =
							this.challengeData.kahoot.questions[
			...
			case "answer": {
				...
				if (q.type === "content") {
					...
					if (
						this.data.questionIndex ===
						this.challengeData.kahoot.questions.length
					) {
						this.data.phase = "close";
					}
					if (
						this.challengeData.challenge.game_options
							.question_timer &&
						!this.defaults.options.ChallengeWaitForInput
					) {
						setTimeout(() => {
			...
			case "close": {
				...
				this._emit("Podium", {
					podiumMedalType: ["gold", "silver", "bronze"][
						this._getRank() - 1
					],
				});
				if (this.defaults.options.ChallengeAutoContinue) {
	...
	const joined = (cid) => {
		...
		this._send = async () => {
			throw "This error should not appear unless you are trying to do something silly.";
		};
	};
		...
		if (m.data && m.data.type === "login") {
			...
			return this._httpRequest(
				`https://kahoot.it/rest/challenges/${
					this.challengeData.challenge.challengeId
				}/join/?nickname=${encodeURIComponent(this.name)}`,
				{
					method: "POST",
				},
				true
	...
	this._getNemesis = () => {
		...
		const scores = Array.from(
			this.challengeData.progress.playerProgress.playerProgressEntries
		);
	...
	this._getProgress = (q) => {
		if (typeof q !== "undefined") {
			return this._httpRequest(
				`https://kahoot.it/rest/challenges/${this.challengeData.challenge.challengeId}/progress/?upToQuestion=${q}`,
				null,
				true
			);
		} else {
			return this._httpRequest(
				`https://kahoot.it/rest/challenges/pin/${this.gameid}`,
				null,
				true
			).then((data) => {
				return this._httpRequest(
					`https://kahoot.it/rest/challenges/${data.challenge.challengeId}/progress`,
					null,
					true
				).then((data2) => {
	...
	this._getProgress()
		.then((inf) => {
			...
			if (
				inf.challenge.endTime <= Date.now() ||
				inf.challenge.challengeUsersList.length >=
					inf.challenge.maxPlayers
			) {
