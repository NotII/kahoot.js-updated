...
class Client extends EventEmitter {
	...
	constructor(options) {
		...
		for (let mod in this.defaults.modules) {
			if (
				this.defaults.modules[mod] ||
				this.defaults.modules[mod] === undefined
			) {
				try {
					require("./modules/" + mod + ".js").call(this);
				} catch (err) {}
			}
		}
	...
	async join(pin, name, team) {
		...
		return new Promise((resolve, reject) => {
			this.handlers.JoinFinish = async (message) => {
				...
				if (
					message.channel === "/service/controller" &&
					message.data &&
					message.data.type === "loginResponse"
				) {
					if (message.data.error) {
						reject(message.data);
						if (
							message.data.description.toLowerCase() !==
							"duplicate name"
						) {
						...
						if (settings.gameMode === "team") {
							...
							if (team !== false) {
								...
								try {
									...
									console.log(
										"ERR! Failed to send team members. Retrying"
									);
									try {
										await this.joinTeam(team, true);
									} catch (e) {
										console.log(
											"ERR! Failed to send team members a second time. Assuming the best."
										);
									}
								}
	...
	leave() {
		...
		if (!arguments[0]) {
			this.disconnectReason = "Client Left";
		}
		setTimeout(() => {
			if (!this.socket) {
				return;
		...
		return new Promise((res, rej) => {
			if (data.data && !data.data.isChallenge) {
				const options = this._defaults.wsproxy(
					`wss://kahoot.it/cometd/${this.gameid}/${data.token}`
				);
				...
				if (
					typeof options.readyState === "number" &&
...
Client.prototype._defaults = {
	...
	wsproxy: (url) => {
		return { address: url };
	}, // Take in [WS Options]{@link https://github.com/websockets/ws/blob/HEAD/doc/ws.md#new-websocketaddress-protocols-options}. Return and modify the options for the new proxied websocket connection
	/**
